...
// Matches users who can currently do `module.action` — driven by what each
// role's permission matrix actually grants, so any role qualifies regardless
// of how it's named or scoped (Data Scope is about how much data a role can
// see, not what type of employee it is — a role literally named "salesman"
// with Data Scope left on "all" would silently vanish from these lists under
// the old dataScope-based check). Falls back to the legacy `role` enum for
// users the RBAC backfill deliberately left with roleId = null (see
// scripts/backfill-roles.js) — without it these queries return nothing for
// anyone not yet manually reassigned a Role. Remove the fallback once every
// user has been migrated to a Role.
async function whereModulePermission(module, action, legacyRoles = []) {
  const roles = await prisma.role.findMany({ select: { id: true, permissions: true } });
  const roleIds = roles.filter((r) => r.permissions?.[module]?.[action]).map((r) => r.id);
  return {
    OR: [
      { roleId: { in: roleIds } },
...
app.post('/api/payroll/generate', authMiddleware, requirePermission('payroll', 'view'), async (req, res) => {
  try {
    ...
    const employees = await prisma.user.findMany({ where: { NOT: await whereModulePermission('roles', 'edit', ['stock_manager']) } });
...
app.post('/api/stock-update-requests', authMiddleware, requirePermission('stockRequests', 'create'), async (req, res) => {
  try {
    ...
    // Notify all admins
    const admins = await prisma.user.findMany({ where: await whereModulePermission('roles', 'edit', ['stock_manager']) });
