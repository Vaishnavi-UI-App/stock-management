...
import { Download, Eraser, Upload, Camera, Share2 } from 'lucide-react';
...
export function DealerApplication() {
  ...
  const [files, setFiles] = useState<Record<FileField, string | null>>({
    photo: null, stamp: null, cheque: null
  });
  // Dealer signature (main)
  const [signature, setSignature] = useState<string>('');
  const [drawing, setDrawing] = useState(false);
  const signCanvasRef = useRef<HTMLCanvasElement>(null);

  // TSO signature (Territory Sales Officer)
  ...
  const shareWhatsApp = async () => {
    ...
    // Fallback: download + open WhatsApp with company number
    pdf.save(fileName);
    const companyWhatsApp = '917020455358'; // Dynamic Crop Science
    ...
    <div className="stock-page">
      <div className="page-header">
        <div>
          ...
          <p>Fill dealership application form and generate PDF</p>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button className="btn btn-success" onClick={shareWhatsApp} style={{ background: '#25D366', borderColor: '#25D366' }}>
            <Share2 size={16} /> Share to Company WhatsApp
          </button>
        </div>
      </div>
      ...
      <div ref={page1Ref} style={{ background: '#fff', padding: '24px 32px', maxWidth: 800, margin: '0 auto 20px', border: '1px solid #e0e0e0', fontFamily: 'Arial, sans-serif' }}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input className="form-input" value={form.firmName} onChange={e => set('firmName', e.target.value)} placeholder="Name of the Firm" style={{ marginBottom: 6, fontSize: 13 }} />
          <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
            <span style={labelStyle}>*Full Address of the Firm :-</span>
            <div style={{ ...fieldLine, flex: 1 }}>{form.fullAddress}</div>
          </div>
          <input className="form-input" value={form.fullAddress} onChange={e => set('fullAddress', e.target.value)} placeholder="Full Address of the Firm" style={{ marginBottom: 6, fontSize: 13 }} />
          <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
          ...
          <input className="form-input" value={form.firmType} onChange={e => set('firmType', e.target.value)} placeholder="Own / Partnership / LLP / Pvt.Ltd. / Other" style={{ marginBottom: 6, fontSize: 13 }} />
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 6 }}>
            <div><span style={labelStyle}>*Mobile No.:</span><input className="form-input" value={form.mobile} onChange={e => set('mobile', e.target.value)} style={{ fontSize: 13 }} /></div>
            <div><span style={labelStyle}>*Telephone No.:</span><input className="form-input" value={form.telephone} onChange={e => set('telephone', e.target.value)} style={{ fontSize: 13 }} /></div>
          </div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
            <span style={labelStyle}>*E-mail address:</span>
            <input className="form-input" value={form.email} onChange={e => set('email', e.target.value)} style={{ flex: 1, fontSize: 13 }} />
          </div>
        </div>
            ...
            <tbody>
              {[1, 2, 3].map(i => (
                <tr key={i}>
                  ...
                  <td style={cellBorder}><input className="form-input" style={{ border: 'none', fontSize: 12, padding: '2px 4px' }} value={(form as any)[`partner${i}Name`]} onChange={e => set(`partner${i}Name` as keyof typeof form, e.target.value)} /></td>
                  <td style={cellBorder}><input className="form-input" style={{ border: 'none', fontSize: 12, padding: '2px 4px' }} value={(form as any)[`partner${i}Address`]} onChange={e => set(`partner${i}Address` as keyof typeof form, e.target.value)} /></td>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input className="form-input" value={form.residenceAddress} onChange={e => set('residenceAddress', e.target.value)} style={{ fontSize: 13 }} />
        </div>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <div><span style={labelStyle}>*Pan No.:-</span><input className="form-input" value={form.pan} onChange={e => set('pan', e.target.value)} style={{ fontSize: 13 }} /></div>
            <div><span style={labelStyle}>*GST Registration No.:-</span><input className="form-input" value={form.gst} onChange={e => set('gst', e.target.value)} style={{ fontSize: 13 }} /></div>
            <div><span style={labelStyle}>*License No.:-</span><input className="form-input" value={form.license} onChange={e => set('license', e.target.value)} style={{ fontSize: 13 }} /></div>
            <div><span style={labelStyle}>*Udyam Registration No.:-</span><input className="form-input" value={form.udyam} onChange={e => set('udyam', e.target.value)} style={{ fontSize: 13 }} /></div>
          </div>
          <div style={{ marginTop: 6 }}>
            <span style={labelStyle}>*Validity:-</span>
            <input className="form-input" value={form.validity} onChange={e => set('validity', e.target.value)} style={{ fontSize: 13 }} />
          </div>
        </div>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 6 }}>
            <div><span style={labelStyle}>*Turnover in Last year Wholesaler:-</span><input className="form-input" value={form.turnoverWholesaler} onChange={e => set('turnoverWholesaler', e.target.value)} style={{ fontSize: 13 }} /></div>
            <div><span style={labelStyle}>*Turnover Retailer:-</span><input className="form-input" value={form.turnoverRetailer} onChange={e => set('turnoverRetailer', e.target.value)} style={{ fontSize: 13 }} /></div>
          </div>
          <div style={{ marginTop: 6 }}>
            <span style={labelStyle}>*Date of Year of establishment of Firm:-</span>
            <input className="form-input" value={form.establishmentDate} onChange={e => set('establishmentDate', e.target.value)} style={{ fontSize: 13 }} />
          </div>
        </div>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <div><span style={labelStyle}>*Name of the Bank:-</span><input className="form-input" value={form.bankName} onChange={e => set('bankName', e.target.value)} style={{ fontSize: 13 }} /></div>
            <div><span style={labelStyle}>*Account No.:-</span><input className="form-input" value={form.accountNo} onChange={e => set('accountNo', e.target.value)} style={{ fontSize: 13 }} /></div>
            <div><span style={labelStyle}>*Branch:-</span><input className="form-input" value={form.branch} onChange={e => set('branch', e.target.value)} style={{ fontSize: 13 }} /></div>
            <div><span style={labelStyle}>*IFCS Code :-</span><input className="form-input" value={form.ifsc} onChange={e => set('ifsc', e.target.value)} style={{ fontSize: 13 }} /></div>
          </div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {[1, 2, 3].map(i => (
                <tr key={i}>
                  <td style={cellBorder}>{i}</td>
                  <td style={cellBorder}><input className="form-input" style={{ border: 'none', fontSize: 12, padding: '2px 4px' }} value={(form as any)[`company${i}Name`]} onChange={e => set(`company${i}Name` as keyof typeof form, e.target.value)} /></td>
                  <td style={cellBorder}><input className="form-input" style={{ border: 'none', fontSize: 12, padding: '2px 4px' }} value={(form as any)[`company${i}Product`]} onChange={e => set(`company${i}Product` as keyof typeof form, e.target.value)} /></td>
                  <td style={cellBorder}><input className="form-input" style={{ border: 'none', fontSize: 12, padding: '2px 4px' }} value={(form as any)[`company${i}Turnover`]} onChange={e => set(`company${i}Turnover` as keyof typeof form, e.target.value)} /></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 16, marginBottom: 8 }}>
            <div><span style={labelStyle}>*Cheque No. 1)</span><input className="form-input" value={form.chequeNo1} onChange={e => set('chequeNo1', e.target.value)} style={{ width: 120, fontSize: 13 }} /></div>
            <div><span style={labelStyle}>2)</span><input className="form-input" value={form.chequeNo2} onChange={e => set('chequeNo2', e.target.value)} style={{ width: 120, fontSize: 13 }} /></div>
            <div><span style={labelStyle}>3)</span><input className="form-input" value={form.chequeNo3} onChange={e => set('chequeNo3', e.target.value)} style={{ width: 120, fontSize: 13 }} /></div>
          </div>
        </div>
      ...
      <div ref={page2Ref} style={{ background: '#fff', padding: '24px 32px', maxWidth: 800, margin: '0 auto 20px', border: '1px solid #e0e0e0', fontFamily: 'Arial, sans-serif' }}>
        ...
        <div style={{ border: '1px solid #333', padding: 16, marginTop: 12 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <input className="form-input" value={form.bankNameSecurity} onChange={e => set('bankNameSecurity', e.target.value)} placeholder="Bank Name for Security" style={{ fontSize: 13, marginTop: 4 }} />
          </div>
          ...
          <div style={{ display: 'flex', gap: 24, marginBottom: 12, flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 200 }}>
              ...
              <input className="form-input" value={form.securityAmount} onChange={e => set('securityAmount', e.target.value)} style={{ fontSize: 13 }} />
            </div>
            <div style={{ flex: 1, minWidth: 200 }}>
              <span style={labelStyle}>Details for RTGS/NEFT/DD/Cheque No.:-</span>
              <input className="form-input" value={form.rtgsDetails} onChange={e => set('rtgsDetails', e.target.value)} style={{ fontSize: 13 }} />
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 4 }}>
                ...
                <input className="form-input" value={form.dealerPlace} onChange={e => set('dealerPlace', e.target.value)} style={{ width: 200, fontSize: 13 }} />
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 4 }}>
                <span style={labelStyle}>Date :-</span>
                <input className="form-input" type="date" value={form.dealerDate} onChange={e => set('dealerDate', e.target.value)} style={{ width: 200, fontSize: 13 }} />
              </div>
            </div>
          ...
          <div style={{ borderTop: '2px solid #333', paddingTop: 16, marginTop: 16 }}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <textarea className="form-input" value={form.remark} onChange={e => set('remark', e.target.value)} rows={2} style={{ fontSize: 13 }} />
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 40, alignItems: 'flex-end' }}>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 12, color: '#555' }}>{salesmanName || 'Salesman Name'}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto 40px', display: 'flex', gap: 12, justifyContent: 'center' }}>
